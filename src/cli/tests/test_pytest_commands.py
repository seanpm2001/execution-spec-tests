...
import cli.pytest_commands

from ..pytest_commands import default_html_report_name, fill
...
class TestHtmlReportFlags:
    ...
    def fill_args(self):
        ...
        Specifies a single existing example test case for faster fill execution, and to allow for
        tests to check for the fixture generation location.
        """
    ...
    def monkeypatch_default_output_directory(self, monkeypatch, temp_dir):
        ...
        def mock_default_fill_output_directory():
            return temp_dir

        monkeypatch.setattr(
            cli.pytest_commands,
            "default_fill_output_directory",
            mock_default_fill_output_directory,
        )
    ...
    def test_fill_default_output_options(
        ...
        """
        Tests the default pytest html report and output generation.
        """
        default_html_path = temp_dir / default_html_report_name("fill")
    ...
    def test_fill_no_html_option(
        ...
        """
        Tests pytest html report is disabled with the `--no-html` flag.
        """
        default_html_path = temp_dir / default_html_report_name("fill")
        fill_args += ["--no-html"]
        result = runner.invoke(fill, fill_args)
    ...
    def test_fill_output_option(
        ...
        non_default_html_path = output_dir / default_html_report_name("fill")
        fill_args += ["--output", output_dir]
        result = runner.invoke(fill, fill_args)
        assert result.exit_code == pytest.ExitCode.OK
        assert non_default_html_path.exists()
    ...
    def test_fill_html_and_output_options(
        ...
        fill_args += ["--output", output_dir, "--html", html_path]
        result = runner.invoke(fill, fill_args)
        assert result.exit_code == pytest.ExitCode.OK
